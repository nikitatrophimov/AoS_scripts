"""
Originally written by Triplefox, i've just added '/coords' command
"""
...
def coords(self):
    if not self.squad:
	self.send_chat("You're not in any squad yet")
	return
    ...
    for n in xrange(len(all_members)):
        result_log_message.append(membernames[n] + ": " + getSquareByCoordinates(all_members[n].get_location()))
    self.send_lines(result_log_message) 
...
for func in (squad, follow, coords):
    add(func)
